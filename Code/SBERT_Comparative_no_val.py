...
def train_and_test_iterative(dataname, N=1, itr=20):
    ...
    train_list_og = pd.concat([train_list_og, val_list_og], axis=0)  # Combine training and validation sets
    ...
    for i in range(itr):
        ...
        checkpoint_path = "checkpoint/comp_mixval.keras"
        ...
        # Train model
        history = de.fit(features, features["Label"],
...
for d in datas:
    ...
    for n in [1,2,3,4,5,10]:
        ...
        for r in temp_r:
            ...
            results.append(
                {"Data": d, "N": n, "Pearson Train": r_train, "Spearman Train": rs_train, "Pearson Test": r_test,
                 "Spearman Test": rs_test})
results = pd.DataFrame(results)
print(results)
results.to_csv("../Results/SBERT-Comparative (without validation).csv", index=False)
